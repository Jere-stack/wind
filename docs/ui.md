...
> **Luvut ovat vanhentuneet.** Palkki on nyt 52 px ja käyrä
> `.20/.38/.28/.14`; vaihtoehto 1 ("kasvata janaa pystysuunnassa")
> tehtiin lopulta, koska jakauma mitattiin uudelleen. Ks.
> *Aikajana Applen mitalla*.
...
## Aikajana Applen mitalla: korkeampi palkki, kupu ja kalenterikisko

Käyttäjän pyyntö oli kolmiosainen: korkeampi jana jossa palkkien ero
näkyy, liukuväri play- ja kelihyppynapin taustaan, ja nätimpi
päivävalitsin. Muiden sääappien katsaus antoi kaksi asiaa joita
kannatti lainata: Windy.app niputtaa tuntipalkit väriksi ja korkeudeksi
samaan tapaan kuin tämä jana jo tekee, ja iOS 26:n "hierarchy through
depth" sanoo että tärkeys tulee syvyydestä eikä väristä — mikä sopii
tähän projektiin, jossa kirkkaus ja kylläisyys on jo varattu datalle.

### 1. Palkki 35 → 52 px, ja käyrä uudelleen

Ensin mitattiin mitä siitä on hyötyä. Peräkkäisten tuntien ero
varastossa (n = 1 116):

| | m/s | 35 px | 52 px |
|---|---|---|---|
| mediaani | 0,27 | 0,70 px | 1,04 px |
| p75 | 0,47 | 1,4 px | 2,1 px |
| p90 | 0,73 | 2,2 px | 3,3 px |

Mediaania ei voi korkeudella pelastaa: se vaatisi 15 px/(m/s) eli
170 px:n palkin. Mutta ylempi puolisko nousee näkyväksi, ja se on se
osa jakaumasta jossa keli vaihtuu.

Sama mittaus paljasti että **käyrä oli väärässä paikassa**. Varaston
tuulijakauma (8 paikkaa, 1 496 tuntia) vasten vanhaa asteikkoa:

| väli | osuus tunneista | osuus korkeudesta (vanha) |
|---|---|---|
| 0–4 m/s | 41,9 % | 15 % |
| 4–8 | 42,0 % | 35 % |
| 8–12 | 15,8 % | 30 % |
...
Viidennes korkeudesta meni kolmelle promillelle tunneista. Uusi jako
`.20 / .38 / .28 / .14` siirtää sen alaspäin:

```
     0- 4 m/s -> 0,00-0,20   2,60 px / (m/s)   (oli 1,31)
     4- 8     -> 0,20-0,58   4,94 px           sessioraja (oli 3,06)
     8-12     -> 0,58-0,86   3,64 px           paras keli (oli 2,63)
    12-16     -> 0,86-1,00   1,82 px
       > 16   -> 1,00        kyllästyy, väri jatkaa
```

Kyllästyminen 16 m/s:ssä säilyy ja myrskytunti on yhä pisin palkki —
sen viimeiset neljä metriä vain tiivistyvät, ja väri jatkaa siitä.
Puuskahunnun katto skaalattiin samassa suhteessa (9 → 13 px), muuten
huntu olisi ollut suhteessa kolmanneksen matalampi kuin ennen.

**Kortin korkeus ei ole enää oma lukunsa.** Tuntilukema on
absoluuttisesti tikin yläreunassa ja palkki kasvaa pohjasta ylös, joten
niiden väliin on varattava tila käsin. Ensimmäinen versio kasvatti
palkin 52:een mutta kortin vain 103:een, ja mittaus näytti mitä siitä
seuraa:

| | rako lukemaan |
|---|---|
| ruudulla nähty maksimi 43,8 px | 16 px (mobiili) / 14 px (työpöytä) |
| täysimittainen 52 px | **3 px / 1 px** |

Eli vika olisi näkynyt vain myrskyssä — juuri silloin kun janaa
luetaan. Nyt `--tl-palkki-h` on yksi muuttuja ja kortti lasketaan siitä
(`palkki + 56`); JS lukee saman luvun `getComputedStyle`lla. Mitattu
jälkeen: 8 px mobiilissa, 6 px työpöydällä. Kortti 88 → 108 px eli
2,3 % iPhonen ruudusta.

### 2. Kiekko on kupu, ei lätty

Projektin oma sääntö kieltää napin tummentamisen: se tekisi
kontrollista kortin äänekkäimmän elementin datan päällä. Liukuväri
kiertää sen, koska se on **materiaali eikä äänenvoimakkuus**: ylhäällä
valo, alhaalla varjo, ja keskikohta jää täsmälleen `--surface-hi`:ksi.

Mitattuna ruudun pikseleistä: kiekon keskiarvokirkkaus 0,8367 ja kortin
paperi 0,8013 eli **1,04:1** — nappi ei ole yhtään kovempi kuin ennen.
Kuvun ylä- ja alapuoliskon ero on **1,093:1**.

> Mittari piti korjata ensin: koko kiekosta laskettu minimi ja maksimi
> osuivat play-kuvakkeeseen (musta) eikä liukuväriin. Oikea mittari on
> rengas, joka ohittaa kuvakkeen.

`background-color` kantaa värin ja `background-image` muodon. Työnjako
on tahallinen: `.playing` vaihtaa vain värin (`--accent`) ja sama kupu
jää voimaan. Shorthand `background` nollaisi kuvan. Käytöstä poissa
oleva kelihyppy sammuttaa kuvun (`background-image: none`) ja on litteä
levy — sama esine, ei nostetta.

### 3. Päiväkisko on kalenteri

Yksi rivi ("Ke 10.") vaihtui kahdeksi (viikonpäivä pienenä numeron
päällä) — sama jäsennys kuin Applen kalenterissa. Mitattu seuraus ei
ole kosmeettinen:

| | ennen | nyt |
|---|---|---|
| lapun leveys | ~64 px | 38 px |
| yhdeksän päivän kisko | 576 px = 1,7 ruudullista | 342 px = **mahtuu kokonaan** |

Kisko on olemassa raahauksen välttämiseksi, ja nyt viikon päähän ei
tarvitse vierittää edes kiskoa.

Leveys on `min-width`, ei tekstin mitta: pelkällä täytteellä
yksinumeroinen päivä olisi 28 px ja kaksinumeroinen 32 px, ja
kalenterikiskossa epätasaiset sarakkeet näkyvät heti. 38 px on
kompromissi — kapeampi olisi mahduttanut enemmän päiviä mutta tehnyt
jokaisesta lapusta huonomman maalin. Kaikki seitsemän napautuskohtaa
(keskeltä ja kulmista) osuvat.

"Tänään" ei ole enää sana vaan **pehmeä pilleri** (`--hairline`).
Kaksi tasoa, molemmat neutraaleja: tänään on vaimea pinta, valittu on
tumma pilleri. `--accent` olisi ollut väärä, koska se on toiminto- ja
varoitusväri. Aria-nimi säilyttää täyden muodon ("Ke 10.9. (tänään)"),
koska "KE / 10" luettuna ei ole päivämäärä.

Työpöydällä kisko keskitetään (`justify-content: safe center`).
Pelkkä `center` tekisi ylivuotavan sisällön ensimmäisestä lapusta
saavuttamattoman; `safe` palaa alkuun heti kun sisältö ei mahdu, ja
selain joka ei tunne avainsanaa hylkää koko säännön eli päätyy entiseen
`flex-start`iin.

### Mitä EI tehty

- **Palkkien tihentäminen tai meteogrammikäyrä.** Molemmat on kaadettu
  jo mittauksella aiemmin (ks. *Aikajanan ura vaihtui hiekkaan* ja
  CLAUDE.md:n aikajanasäännöt).
- **Päivälapun tuulikaista takaisin.** Kisko on navigointia; sama tieto
  on tuntirivillä tarkempana.
- **Uran palautus palkkien alle.** Kortti on yhä yhtä paperia.
